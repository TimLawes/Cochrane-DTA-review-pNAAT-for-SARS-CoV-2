...
# Use 'madad' function 
confmatrix <- metadata %>% dplyr::select(test_eval_id,t_p,f_n,f_p,t_n, pool_size_gp, disease_p, disease_n, pool_size, pnaat, naat, sample, pool_type)
confmatrix <- confmatrix %>% rename(c(Study = test_eval_id, TP=t_p, FN=f_n, FP=f_p, TN=t_n))
confmatrix <- confmatrix %>% mutate (fpr = FP/disease_n)
confmatrix <- confmatrix %>% mutate (point = TP/disease_p)
confmatrix <- confmatrix %>% mutate (n=TP+FP+TN+FN)
hg <- madad(confmatrix)
...
```{r message=FALSE, warning=FALSE}
# Descriptive plots
p1cor <- ggplot (confmatrix, aes(x= fpr, y=point, size=n))+
...
```{r message=FALSE, warning=FALSE}
cts<- crosstable_statistics(data=confmatrix, x1=fpr, x2=point, statistics="spearman", weights= corrfprse$totalsamples)
...
summary(R2_9parts)
alt <- cbind (R2_9parts$IR2_boot, R2_9parts$BW_boot)
```
```{r message=FALSE, warning=FALSE}
alt <- R2_9parts$IR2
alt
...
sampletype2 <- ggemmeans(ysemp9, terms = c("log2p", "sample", "naat"), type="fe")
