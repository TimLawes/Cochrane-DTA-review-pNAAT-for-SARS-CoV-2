...
library(splines)
library(RVAideMemoire) # For Spearman's rank with boostrap 95% CI
```
...
# Use 'madad' function 
confmatrix <- metadata %>% dplyr::select(test_eval_id,t_p,f_n,f_p,t_n)
...
### 3.1.3 Check for threshold effects

#### Correlation of Sensitivity and FPR
We investigated a threshold effect (i.e a trade-off of sensitivity and specificity as positivity threshold is varied) using the *spearman.ci* function of the *RVAideMemoire* package. This provides a summary estimate of the correlation between sensitivity and false-positive rate (1-specificity), and boostrapped 95% CI. We set boostrapping to 500 replications.

```{r message=FALSE, warning=FALSE}

# Collate sensitivity and FPR predictions from 
corrfprse <- metadata %>% dplyr::select(f_p,t_p,f_n, t_n, disease_p, disease_n, totalsamples)
corrfprse <- corrfprse %>% mutate(fpr=f_p/disease_n)
corrfprse <- corrfprse %>% mutate(point=t_p/disease_p)
corrfprse <- tibble::rowid_to_column(corrfprse, "ID")
corrfprse$ID <- as.character(corrfprse$ID)
corrfprse <- corrfprse %>% mutate(ID = fct_reorder(ID, desc(n)))

# Get Spearman's Rank correlation and covariance
sprank<- spearman.ci(corrfprse$fpr, corrfprse$point, nrep = 500, conf.level = 0.95)
values <- c(sprank[[(5)]],sprank[[(6)]])
parameter<- c("rho","lower CI","upper CI")
cor <- cbind(parameter,values)
cor <- as.tibble(cor)
gt_cor <-gt(cor)
gt_cor %>% tab_header(title = "Correlation of sensitivity and FPR")
```

#### Descriptive plots
Descriptive plots of observed and estimated sensitivity and 1-specificity (FPR) in ROC space confirm the lack of association, given specificity is ~100% in all studies. While the pool-test stage of a pooling method can result in 'FP' pools, subsequent re-testing of samples within these pools, or analoguous algorithmic deconvoluting, largely excludes FP individual sample results.
...
#### Reitsma et al approach to fitting summary point and 95% CI
We used the bivariate linear mixed model of Reitsma et al to obtain a
```{r message=FALSE, warning=FALSE}
confmatrix <- confmatrix %>% filter(!Study %in% 
   c('Ben-Ami 2020 [B]', 	'Chakravarthy 2020 [A]', 	'Chakravarthy 2020 [B]', 'Chakravarthy 2020 [C]'))
confmatrix <- confmatrix %>% mutate(fpr = FP/(FP+TN))
confmatrix <- confmatrix %>% mutate(sens = TP/(TP+FN))
reitsma <- reitsma(confmatrix)
plot (reitsma, xlim = c(0,0.1), ylim = c(0,1))
points(fpr(confmatrix), sens(confmatrix), xlim = c(0,0.02), ylim = c(0,1), pch = 3, Add=TRUE)

```

#### Bayesian approach to fitting summary point and 95% CI
We used the *metadiag* function from the *bamdit* package to take a Bayesian approach to fitting a summary point and
```{r message=FALSE, warning=FALSE}
...
write_csv (confmatrix2, "matrix.csv")

result <- metadiag(confmatrix2, two.by.two=TRUE, re = "normal", re.model = "DS", link = "logit", sd.Fisher.rho = 1.7, nr.burnin = 1000, nr.iterations = 10000, nr.chains = 4, r2jags = TRUE)

plot(result, level = 0.95, parametric.smooth = TRUE)
...
Based on these observations we used the glmer function in "lme4" to fit:
...
Use glmer to fit an integer only bivariate binomial multi-level (random-effects) meta-analysis with an unstructured covariance matrix. We observe
...
ysemp2_sum
co4b <- ysemp2_sum$coeff[1,1]
ci4b <- confint  (ysemp1)
plogis(co4b)
plogis(ci4b)
```

##### Nested (studies nested within cohorts)
```{r message=FALSE, warning=FALSE}
...
ysemp3_sum
co4c <- ysemp1_sum$coeff[1,1]
ci4c <- confint  (ysemp1)
plogis(co4c)
plogis(ci4c)
```
##### Compare sensitivity only models
```{r message=FALSE, warning=FALSE}

lrtest(ysemp3,ysemp2)
```
Observations
...
ypool2 <- glmer (formula= cbind(true,n-true)~0+sens+spec+
(0+sens|testeval)+(0+spec|testeval), data = ymp2,family=binomial,nAGQ=1,verbose=0) # fixed + random model
ypool2_sum <- summary(ypool2)
ypool2_sum
# Obtain sensitivity and 95% CI using simulation and boostrap options
feEx <- FEsim(ypool2, 1000) #Obtain fixed effects through simulation
cbind(feEx[,1] , round(feEx[, 2:4], 3))

# Obtain specficity and 95% CI using binom.test as no model convergence with bivariate approach
yp2sens <-c(binom.test(251,264)$estimate, binom.test(251,264)$conf.int)
yp2spec <-c(binom.test(264,264)$estimate, binom.test(264,264)$conf.int)
yp2sens 
yp2spec 


PI <- predictInterval(merMod = ypool2, newdata = ymp2,
...
## 5.2 NAAT type and Pool size groups and NAAT type
