...
```{r message=FALSE, warning=FALSE}
mp2b <- glmer (formula= cbind(true,n-true)~ 1+(1|testeval), data = ysemp2, family=binomial,nAGQ=1,verbose=0) # fixed + random model
...
mp2c <- glm (formula= cbind(true,n-true)~ 1, data = ysemp2, family=binomial) # fixed + random model
mp2c_sum <- summary(mp2c)
mp2c_sum
set.seed (212)
mp2c_ci <- confint (mp2c, level=0.99, method="boot", quiet=TRUE)
...
summp47 <- cbind(lab47,est47,ci47)
gt47 <- gt(as.tibble(summp47))
gt47
```
...
#### Stage 2 model
```{r message=FALSE, warning=FALSE}
mp16b <- glmer (formula= cbind(true,n-true)~ 1+(1|testeval), data = ymp16se, family=binomial,nAGQ=1,verbose=0) # fixed + random model
...
lab16 <- c("Sensitivity")
sum16<- c(lab16,est16,ci16)
gt16 <- gt(as.tibble(sum16))
gt16
```
...
### Pool size 32+
Only 1 study was included with a pool size 32+. We therefore summarised by binom.test and binomial exact CI (99%)
...
cosum <- rbind(ymp32sens,ymp32spec)
rowlab <- c("Sensitivity","specificity")
comb_summary <-cbind(rowlab,cosum)
comb<- as.tibble(comb_summary)
comb<- comb %>% rename(Parameter = rowlab, estimate ="probability of success", "lower 95% CI"=V3, "upper 95% CI"=V4)
